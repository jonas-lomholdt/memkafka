...
docker run --detach \
  ...
  --env KAFKA_LISTENERS=PROXY://:19092,DIRECT://:19094,CONTROLLER://:19093 \
  --env KAFKA_ADVERTISED_LISTENERS=PROXY://api-version-proxy:9092,DIRECT://kafka:19094 \
  --env KAFKA_CONTROLLER_LISTENER_NAMES=CONTROLLER \
  --env KAFKA_LISTENER_SECURITY_PROTOCOL_MAP=CONTROLLER:PLAINTEXT,PROXY:PLAINTEXT,DIRECT:PLAINTEXT \
  --env KAFKA_INTER_BROKER_LISTENER_NAME=DIRECT \
...
docker run --detach \
  --name "${SEED_CONTAINER}" \
  --network "${NETWORK}" \
  --env MEMKAFKA_BOOTSTRAP_SERVERS=kafka:19094 \
