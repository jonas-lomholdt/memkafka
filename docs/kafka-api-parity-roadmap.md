...
| 3 | Metadata | 4-13 | 0-13 | Real clients prove discovery, advertised address, auto-creation, explicit partition counts, and all-topic listing. Java name-based Admin calls compare stable non-zero IDs from CreateTopics, ListTopics, and DescribeTopics; focused Rust and TCP wire tests prove Metadata UUID lookup by name and ID. | No rack data, listener/security semantics, deletion/recreation lifecycle, or realistic cluster changes. |
...
| 60 | DescribeCluster | 2 | 0-2 | Java 4.3.1 `Admin.describeCluster()` proves the cluster ID and matching broker/controller ID; TCP wire tests prove the exact listener-specific advertised host and port. | Historical v0-v1 and security authorization fields are intentionally outside the current window. |
...
- return typed errors for every schema-known adjacent version edge without widening any advertised API's supported window;
- exercise an exhaustive Rust matrix of every advertised floor and ceiling, every schema-known adjacent typed rejection, and connection-survival paths, plus focused flexible/tagged and rejected-mutation coverage;
